...
def solution_one_third (nums, target):
...
def solution_two (nums, target):
    ...
    for index, i in enumerate(nums):
        difference = target - i
        if difference in dicts:
            if (difference == i) and len(dicts[difference]) > 1:
                return dicts[difference]
            elif dicts[difference][0] == index:
                continue
            else:
                return [dicts[difference][0], index]